...
## Bull Auto-Battle Controller
## Melee tank — slow but high HP, charges enemies and deals direct damage
...
@onready var anim = $BullAnimation
...
func _get_separation_force(to_target: Vector2) -> Vector2:
	...
	for hero in get_tree().get_nodes_in_group("heroes"):
		...
		if dist < SEPARATION_RADIUS and dist > 0.0:
			...
			# If ally is in our path to target, steer perpendicular to go around them
